...
do
    for lr in "${PEAK_LRS[@]}"
        do
        CHECKPOINT_SUFFIX=xai_apex_${size}_${lr}_filtered.pt
        MUSICBERT_PATH=checkpoints/checkpoint_last_musicbert_${size}_filtered.pt
    
        fairseq-train xai_data_bin_apex_filtered/0 --user-dir musicbert \
            ...
            --tensorboard-logdir /data1/jongho/muzic/musicbert/checkpoints/board_apex_${lr}_${size}_${BATCH_SIZE}_filtered \
...
do
    ...
    for size in "${SIZES[@]}"
    do
        CHECKPOINT_SUFFIX=xai_apex_${size}_${lr}_${BATCH_SIZE}_adv_filtered.pt
        ...
        for lr in "${PEAK_LRS[@]}"
            ...
            fairseq-train xai_data_bin_apex_filtered/0 --user-dir musicbert \
                ...
                --lr-scheduler polynomial_decay --lr $lr --total-num-update $TOTAL_NUM_UPDATES --warmup-updates $WARMUP_UPDATES \
                --log-format json --log-interval 70 \
                --tensorboard-logdir /data1/jongho/muzic/musicbert/checkpoints/board_apex_${lr}_${size}_${BATCH_SIZE}_adv_filtered \
                --best-checkpoint-metric R2 \
                --shorten-method "truncate" \
                --checkpoint-suffix _${CHECKPOINT_SUFFIX} \
                --no-epoch-checkpoints \
