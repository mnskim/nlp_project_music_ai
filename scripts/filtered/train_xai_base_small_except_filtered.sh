...
do
    for lr in "${PEAK_LRS[@]}"
        do
        CHECKPOINT_SUFFIX=xai_apex_${size}_${lr}_except_filtered.pt
        MUSICBERT_PATH=checkpoints/checkpoint_last_musicbert_${size}_except_filtered.pt
    
        fairseq-train xai_data_bin_apex_except_filtered/0 --user-dir musicbert \
            ...
            --tensorboard-logdir /data1/jongho/muzic/musicbert/checkpoints/board_apex_${lr}_${size}_${BATCH_SIZE}_except_filtered \
...
do
    ...
    for size in "${SIZES[@]}"
    do
        CHECKPOINT_SUFFIX=xai_apex_${size}_${lr}_${BATCH_SIZE}_adv_except_filtered.pt
        ...
        for lr in "${PEAK_LRS[@]}"
            ...
            fairseq-train xai_data_bin_apex_except_filtered/0 --user-dir musicbert \
                ...
                --init-token 0 --separator-token 2 \
                --arch musicbert_${size} \
                --criterion sentence_prediction_multilabel_xai_adv \
                --classification-head-name $HEAD_NAME \
                --num-classes 13 \
                --dropout 0.1 --attention-dropout 0.1 --weight-decay 0.01 \
                --optimizer adam --adam-betas "(0.9, 0.98)" --adam-eps 1e-6 --clip-norm 0.0 \
                --lr-scheduler polynomial_decay --lr $lr --total-num-update $TOTAL_NUM_UPDATES --warmup-updates $WARMUP_UPDATES \
                --log-format json --log-interval 70 \
                --tensorboard-logdir /data1/jongho/muzic/musicbert/checkpoints/board_apex_${lr}_${size}_${BATCH_SIZE}_adv_except_filtered \
                --best-checkpoint-metric R2 \
                --shorten-method "truncate" \
                --checkpoint-suffix _${CHECKPOINT_SUFFIX} \
                --no-epoch-checkpoints \
