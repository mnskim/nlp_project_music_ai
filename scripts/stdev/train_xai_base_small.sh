...
WARMUP_UPDATES=300
PEAK_LRS=(0.00001 0.0001 0.000001)
...
do
    ...
    MUSICBERT_PATH=checkpoints_segsplit/checkpoint_last_musicbert_${size}.pt

    for lr in "${PEAK_LRS[@]}"
        do
        CHECKPOINT_SUFFIX=xai_stdev_M2PF_${lr}_${size}.pt
        fairseq-train processed/xai_data_bin_stdev/0 --user-dir musicbert \
            ...
            --tensorboard-logdir checkpoints_stdev/M2PF/board_stdev_${lr}_${size} \
            --best-checkpoint-metric R2 \
            --shorten-method "truncate" \
            --save-dir checkpoints_stdev \
