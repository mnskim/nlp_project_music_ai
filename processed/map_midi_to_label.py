...
def midi_label_map_apex_base_split():
    ...
    json.dump(music_label_map_apex, open("midi_label_map_apex_reg_cls_base_split.json", 'w'))

def midi_label_map_stdev():
    ...
    # add pianist info
    for key, annot_list in tqdm(music_label_map_std.items()):
        if key.split("_")[-2] not in PIANIST_MAP:
            PIANIST_MAP[key.split("_")[-2]] = len(PIANIST_MAP)
    print(PIANIST_MAP)
    
    for key, annot_list in tqdm(music_label_map_std.items()):
        music_label_map_std[key].append(PIANIST_MAP[key.split("_")[-2]])

    json.dump(music_label_map_std, open("midi_label_map_stdev.json", 'w'))

"""low features 
Stable beat / Long / Cushioned / Saturated (wet) /Clean /Subtle change std <= 80%"""
def midi_label_map_apex_rm_high_std():
    file = open('/data1/jongho/muzic/musicbert/processed/total.csv', encoding="utf-8")
    csvreader = csv.reader(file)
    header = []
    header = next(csvreader)
    ...
    # kernel density estimation
    for key, annot_list in tqdm(music_label_map.items()):
        annot_list_t = np.array(annot_list).transpose()
        maxima = np.array([estimate_maxima(row)/7 for row in annot_list_t])
        maxima = maxima.transpose().tolist()
        music_label_map_apex[key] = maxima
        # 
        if len(annot_list) == 1:
    ...
    threshold = np.percentile([np.array(v[:6]) for v in music_label_map_std.values()], 75, axis=0)
    for k, v in music_label_map_std.items():
        if sum(v[:6]) > sum(threshold):
            music_label_map_apex.pop(k)
    print(len(music_label_map_apex))
    # add pianist info
    for key, annot_list in tqdm(music_label_map_apex.items()):
        if key.split("_")[-2] not in PIANIST_MAP:
    ...
    json.dump(music_label_map_apex, open("midi_label_map_apex_reg_cls_rm_highstd.json", 'w'))
